...
def names_to_apa(authors):
    # formatting
    authors = authors.replace('and ', '')
    authors = authors.replace(', ', ',')
    authors = authors.split(',')
    # format --> Last, F., Joyner, D., Burdell, G.
    abbreviated_names = []
    for i in range(len(authors)):
        first_initial = authors[i].split()[0][0]
        last_name = authors[i].split()[1]
        if i == len(authors) - 1:
            abbreviated_names.append("& {}, {}.".format(last_name, first_initial))
        else:
            abbreviated_names.append("{}, {}.".format(last_name, first_initial))
   
   
    return ', '.join(abbreviated_names)
